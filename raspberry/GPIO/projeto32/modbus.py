...
def _hex(data: bytes) -> str:
    """Formata bytes para exibição em hex."""
...
def add_crc(pacote_sem_crc: bytes) -> bytes:
    """Adiciona CRC ao pacote (little-endian)."""
    crc = _calcula_crc(pacote_sem_crc)
    crc_high = crc & 0xFF
    crc_low = (crc >> 8) & 0xFF
    return bytes(pacote_sem_crc) + bytes([crc_high, crc_low])


def check_crc(pacote_completo: list) -> bool:
    """Verifica CRC de um pacote completo (com CRC)."""
    if len(pacote_completo) < 4:
        return False
    payload = pacote_completo[:-2]
    return list(add_crc(bytes(payload))[-2:]) == pacote_completo[-2:]
...
class ModbusRTU:
    ...
    def __init__(self, port: str, baudrate: int = 115200,
        ...
        self._lock = threading.Lock()  # Lock para acesso exclusivo à serial
    ...
    def _log(self, msg: str):
        """Log condicional baseado no verbose."""
        if self.verbose:
            print(msg)
    ...
    def read_registers(self, slave: int, start: int, count: int) -> list[int] | None:
        ...
        # DEBUG
        self._log(f"[DEBUG READ] Matrícula: {_hex(self._matricula)} (len={len(self._matricula)})")
        self._log(f"[DEBUG READ] Payload antes matrícula: {_hex(payload)}")

        frame = add_crc(payload + self._matricula)

        self._log(f"[DEBUG READ] Frame completo: {_hex(frame)}")

        for attempt in range(self.MAX_RETRIES):
            try:
                with self._lock:  # Lock APENAS durante a operação serial
    ...
    def write_registers(self, slave: int, start: int, values: list[int]) -> bool:
        ...
        # DEBUG
        self._log(f"[DEBUG WRITE] Matrícula: {_hex(self._matricula)} (len={len(self._matricula)})")
        self._log(f"[DEBUG WRITE] Payload antes matrícula: {_hex(payload)}")

        frame = add_crc(payload + self._matricula)

        self._log(f"[DEBUG WRITE] Frame completo: {_hex(frame)}")
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                # Fora do lock: validação e decisão
                if len(response) < 8:
                    sleep(0.05 * (attempt + 1))
                    continue
                if not check_crc(list(response)):
                    ...
                    continue
                if response[1] & 0x80:
                    return False  # Exceção: falha definitiva, não retry
                return True
    ...
    # ------------------------------------------------------------------
    # Função para enviar pacote customizado
    # ------------------------------------------------------------------

    def send_custom_packet(self, payload: bytes, expected_response_size: int = 256) -> bytes | None:
        """Envia um pacote customizado com CRC automático."""
        with self._lock:
