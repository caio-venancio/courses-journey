...
Lê as configurações centralizadas de config.json.
"""
...
# ======================================================================
# CARREGAMENTO DA CONFIGURAÇÃO (Cruzamento 1)
# ======================================================================
INTERSECTION_ID = "1"
...
def _connect_to_central():
    ...
    while True:
        try:
            ...
            print(f"[NET] Falha ao conectar: {e} — tentando em 3s")
            time.sleep(3)
        finally:
            with _central_lock:
...
def _make_callbacks(sensor_id: int):
    # Usamos uma lista local para armazenar o tempo.
    # Isso evita travar a thread com o _lock e zera a latência de busca.
    t_a = [None]

    def on_a(channel):
        # Pega o tempo EXATAMENTE na hora que a borda de subida bater
        t_a[0] = time.monotonic()

    def on_b(channel):
        # Pega o tempo de saída imediatamente
        t_b = time.monotonic()
        ...
        # Faz a matemática pesada e SÓ AGORA pega o lock para atualizar o contador
        with _lock:
            state["counts"][sensor_id] += 1
            t_a[0] = None  # Reseta o sensor
            
        ts = datetime.now(timezone.utc).isoformat()
        print(f"[SPD] Sensor {sensor_id}: {speed:.1f} km/h (dt = {dt:.3f}s)")
        
        # Dispara o alerta para o Central se for maior que o limite
