...
Com suporte à CRC customizada do projeto e Thread-Safety.
"""
...
class ModbusRTU:
    ...
    def __init__(self, port: str, baudrate: int = 115200,
        ...
        # O LOCK QUE EVITA COLISÃO NA PORTA SERIAL
    ...
    def read_registers(self, slave: int, start: int, count: int) -> list[int] | None:
        ...
        with self._lock:  # TRAVA EXCLUSIVA
            for attempt in range(self.MAX_RETRIES):
                try:
                    ...
                    if len(response) < expected: continue
                    if not _validate_response(response): continue
                    if response[1] & 0x80: continue
